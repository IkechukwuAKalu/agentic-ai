    ...
    def respond(self, input_text):
        ...
        system_prompt = f"""
            - The persona with the following instruction:
                "{self.persona}. Forget all previous context."
...
class RAGKnowledgePromptAgent:
    ...
    def chunk_text(self, text):
        ...
        while start < len(text) and chunk_id < 10:
        ...
        for i in range(self.max_interactions):
            ...
            if evaluation.lower().startswith("yes"):
                ...
                response = client.chat.completions.create(
                    model=llm_model,
                    messages=[
                        {"role": "system", "content": "You a detailed feedback provider to improve LLM output"},
...
class RoutingAgent():
    ...
    def route(self, user_input):
        ...
        for agent in self.agents:
            ...
            cosine_similarity = np.dot(input_emb, agent_emb) / (np.linalg.norm(input_emb) * np.linalg.norm(agent_emb))
            print(f"Similarity for {agent['name']} is {cosine_similarity}")
...
class ActionPlanningAgent:
    ...
    def extract_steps_from_prompt(self, prompt):
        ...
        system_prompt = f"""
            You are an action planning agent. Using your knowledge, you extract from the user prompt the 
            steps requested to complete the action the user is asking for. You return the steps as a list. 
